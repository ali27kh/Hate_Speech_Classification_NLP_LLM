...
# LSTM Classifier for binary classification using bidirectional LSTM layers
class LSTMClassifier:
    ...
    def predict(self, X):
        # Adjust input dimensions and predict probabilities
        if len(X.shape) == 4:
    ...
    def evaluate_model(self, test_data):
        # Extract test embeddings and labels from DataLoader
        test_embeddings = []
        test_labels = []
        ...
        # Convert to TensorFlow tensors
        test_inputs = tf.constant(test_embeddings, dtype=tf.float32)
